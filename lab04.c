...
void* thread_func(void* arg) { // thread function that takes multiple parameters
    ...
    //     snprintf(buffer, needed, SUCCESS_MSG, pid, data->core_id);
    //   } else {
    //     const size_t needed = snprintf(NULL, 0, FAILURE_MSG, pid, data->core_id);
    //     buffer = malloc(needed);
    //     snprintf(buffer, needed, FAILURE_MSG, pid, data->core_id);
    //   }
...
int main(int argc, char *argv[]){
    ...
    if (s == 0){
        ...
        zsn(sub, rows-1, 0, cols-1);  // Note: rows-1 because the actual data is rows-1
